                    ...
                    <div class="row">
                        <div class="col d-flex align-items-center">
                            ...
                            <div class="nav-scroll">
                                <div class="nav nav-list nav-list-profile" id="myTab" role="tablist">
								    <a class="nav-item nav-link active" id="infos-tab" data-toggle="tab" href="#infos" role="tab" aria-controls="infos">Informations de <?= $player['pseudo']; ?></a>
									<a class="nav-item nav-link" id="sanctions-tab" data-toggle="tab" href="#sanctions" role="tab" aria-controls="sanctions">Sanctions (<?= $sanctions; ?>)</a>
								</div>
                            </div>
                        </div>
                    </div>
			...
			<section class="py-lg-5">
                <div class="container">
                    <div class="row">
						...
						<div class="col-lg-9 order-2 order-lg-1" id="myTabContent">
					      	<div class="tab-pane fade show active" id="infos" role="tabpanel" aria-labelledby="infos">
							<?php if(isset($_POST['test'])){ ?>
							<?php
							$this->load->view('layouts/rcon');
							$this->db->where('server_name', 'Faction');
							$this->db->where('server_type', 'rcon');
							$query = $this->db->get('connect');
							foreach ($query->result() as $row){
							$r = new rcon($row->server_ip,$row->connect__port,$row->connect__password); //create rcon object for server on the rcon port with a specific password
							}
							  if($r->Auth()){ //Connect and attempt to authenticate{
							    $r->rconCommand('say Essai de '.$user['pseudo'].' depuis le profil de '.$player['pseudo'].''); //send a command
							...
							<?php } ?>
						        <form method="POST">
							    	<button type="submit" name="test">Envoyer un /say</button>
								</form>
							</div>
						    <div class="tab-pane fade" id="sanctions" role="tabpanel" aria-labelledby="sanctions">
